...
class King(game : Game, player : Int, m_pos : Pos) extends Piece(game, player, m_pos) {
	...
	override def possible_move() : List[Pos] = {
		...
		if ( already_moved == -1) {
			/* petit roque */
			if( in_board(x+3, y) && game.board(x+3)(y) != null && game.board(x+3)(y).already_moved == -1 ) // La tour n'a pas bougé
			if( in_board(x+2, y) && game.empty_cell(x + 1, y) && game.empty_cell(x + 2, y)) // C'est vide entre le roi et la tour
			if( in_board(x+3, y) && !game.isControlledCell(x + 1, y, player) && !game.isControlledCell(x + 2, y, player)
			...
			/* Grand roque */
			if( in_board(x-4, y) && game.board(x-4)(y) != null && game.board(x-4)(y).already_moved == -1 ) // La tour n'a pas bougé
			if( in_board(x-3, y) && game.empty_cell(x-1, y) && game.empty_cell(x-2, y) && game.empty_cell(x-3, y) ) // C'est vide
			if( in_board(x-4, y) && !game.isControlledCell(x-1, y, player) && !game.isControlledCell(x-2, y, player)
